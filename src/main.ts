...
import { OakService, TokenService, CreditService } from "./finnotech";
...
app.get("/facilityInq", async (req, res) => {
  try {
    const result = await CreditService.facilityInquiry({
      nid: req.query.nid as string,
    });
    return res.json(result);
  } catch (err) {
...
app.get("/backChequeInq", async (req, res) => {
  try {
    const result = await CreditService.backChequeInquiry({
      nid: req.query.nid as string,
    });
    return res.json(result);
  } catch (err) {
